...
export class SearchSubscriber implements EntitySubscriberInterface {
  ...
  private async syncEntity(entityId: number | undefined, type: string) {
    if (!entityId) {
      this.logger.warn(`syncEntity called with no id for type=${type}, skipping`);
      return;
    }
    try {
      switch (type) {
        case 'Post':
          const post = await this.dataSource.getRepository(Post).findOne({ where: { id: entityId }, relations: ['section', 'author'] });
          if (post) await this.searchService.indexPost(post);
          this.logger.debug(`Synced Post #${entityId} to ES`);
          break;
        case 'Ticket':
          const ticket = await this.dataSource.getRepository(Ticket).findOne({ where: { id: entityId } });
        ...
        case 'Message':
          ...
          this.logger.debug(`Message #${entityId} ES sync handled by ChatService, skipping subscriber`);
          break;
        case 'KnowledgeDoc':
          const doc = await this.dataSource.getRepository(KnowledgeDoc).findOne({ where: { id: entityId } });
          if (doc) await this.searchService.indexKnowledge(doc);
          this.logger.debug(`Synced KnowledgeDoc #${entityId} to ES`);
          break;
      }
    } catch (e: any) {
      this.logger.warn(`Failed to sync ${type} #${entityId} to ES: ${e.message}`);
    }
  }
  ...
  private async removeEntity(id: number, type: string) {
    ...
    try {
      ...
      this.logger.warn(`Failed to remove ${type} ${id} from ES: ${e.message}`);
    }
  }
}
