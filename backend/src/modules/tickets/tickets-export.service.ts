...
import { Document, Paragraph, TextRun, ImageRun, Packer, HeadingLevel, AlignmentType, ShadingType, Table, TableRow, TableCell, WidthType, BorderStyle } from 'docx';
...
export class TicketsExportService {
  ...
  async exportChatZip(ticketId: number, userId: number, userRole: string, res: any): Promise<void> {
    const ticket = await this.ticketRepository.findOne({
      where: { id: ticketId },
      relations: ['creator', 'assignee', 'participants', 'messages', 'messages.sender'],
    });
    ...
    for (const msg of sortedMessages) {
      const senderName = msg.sender?.realName || msg.sender?.displayName || msg.senderName || '未知用户';
    ...
    const docChildren: any[] = [
      ...
      new Paragraph({
        children: [
          new TextRun({ text: `创建人: ${creatorName}    接单人: ${assigneeName}`, color: '555555' }),
        ],
      }),
      new Paragraph({
        children: [
          new TextRun({ text: `导出时间: ${exportTime}`, color: '999999', size: 18 }),
        ],
    ...
    for (const msg of sortedMessages) {
      if (msg.isRecalled) {
        docChildren.push(new Paragraph({
          children: [
            new TextRun({ text: '[该消息已被撤回]', italics: true, color: '999999' }),
          ],
        }));
      ...
      docChildren.push(new Paragraph({
        children: [
          new TextRun({ text: `【${senderName}】 ${timeStr}`, bold: true, color: '555555', size: 18 }),
        ],
        alignment: align,
      }));
      ...
      if (msg.type === 'image' || (msg.content && msg.content.includes('/api/files/static/'))) {
      ...
      if (!imageEmbedded) {
        ...
        const textRuns = lines.map((line: string, idx: number) => new TextRun({
          text: line,
          break: idx > 0 ? 1 : 0,
          shading: { type: ShadingType.CLEAR, fill: isMsgFromCreator ? 'E8F4F8' : 'F0F0F0' },
        }));
        docChildren.push(new Paragraph({
          children: textRuns,
          alignment: align,
  ...
  // ==================== 生成结构化处理报告 DOCX ====================
  async exportReport(ticketId: number, userId: number, userRole: string, res: any): Promise<void> {
    const ticket = await this.ticketRepository.findOne({
      where: { id: ticketId },
      relations: ['creator', 'assignee', 'participants', 'messages', 'messages.sender', 'messages.sender.role'],
    });
    if (!ticket) throw new NotFoundException('工单不存在');
    ...
    const creatorName = ticket.creator?.realName || ticket.creator?.displayName || '-';
    const assigneeName = ticket.assignee?.realName || ticket.assignee?.displayName || '-';
    const participantNames = (ticket.participants || []).map(p => p.realName || p.displayName || p.username).join('、') || '-';
    const categoryStr = ticket.category1
      ? [ticket.category1, ticket.category2, ticket.category3].filter(Boolean).join(' → ')
      : (ticket.type || '-');
    const statusText: Record<string, string> = { pending: '待接单', in_progress: '服务中', closing: '待确认关单', closed: '已关闭' };
      ...
      children: [
        new TableCell({
          ...
          children: [new Paragraph({
            children: [new TextRun({ text: label, bold: true, size: 20, font: 'Microsoft YaHei' })],
        ...
        new TableCell({
          ...
          borders: noBorders,
          children: [new Paragraph({
            children: [new TextRun({ text: value, size: 20, font: 'Microsoft YaHei' })],
            spacing: { before: 40, after: 40 },
          })],
        }),
      ],
    ...
    docChildren.push(new Paragraph({ text: '' }));
    docChildren.push(new Paragraph({
      children: [new TextRun({ text: '技术支持工单处理报告', bold: true, size: 36, font: 'Microsoft YaHei', color: '1a1a2e' })],
      heading: HeadingLevel.TITLE,
      alignment: AlignmentType.CENTER,
      spacing: { after: 120 },
    }));
    docChildren.push(new Paragraph({
      children: [new TextRun({ text: ticket.ticketNo, size: 24, color: '6366f1', font: 'Microsoft YaHei' })],
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 },
    }));
    docChildren.push(new Paragraph({
      children: [new TextRun({ text: `导出时间: ${new Date().toLocaleString('zh-CN')}`, size: 18, color: '999999', font: 'Microsoft YaHei' })],
      alignment: AlignmentType.CENTER,
      spacing: { after: 400 },
    }));
    ...
    const infoTable = new Table({
      ...
      rows: [
        ...
        ...(ticket.participants && ticket.participants.length > 0 ? [makeInfoRow('协作专家', participantNames)] : []),
        makeInfoRow('工单状态', statusText[ticket.status] || ticket.status),
        makeInfoRow('创建时间', new Date(ticket.createdAt).toLocaleString('zh-CN')),
        ...(ticket.assignedAt ? [makeInfoRow('接单时间', new Date(ticket.assignedAt).toLocaleString('zh-CN'))] : []),
        ...(ticket.closedAt ? [makeInfoRow('关单时间', new Date(ticket.closedAt).toLocaleString('zh-CN'))] : []),
      ],
    });
    ...
    for (const line of descLines) {
      docChildren.push(new Paragraph({
        children: [new TextRun({ text: line, size: 21, font: 'Microsoft YaHei' })],
        spacing: { after: 60 },
      }));
    }
    ...
    docChildren.push(new Paragraph({
      children: [new TextRun({ text: `以下为完整的工单沟通记录，共 ${sortedMessages.length} 条消息。`, size: 20, color: '666666', italics: true, font: 'Microsoft YaHei' })],
      spacing: { after: 200 },
    }));
    ...
    for (const msg of sortedMessages) {
      if (msg.isRecalled) {
        docChildren.push(new Paragraph({
          children: [new TextRun({ text: '[该消息已被撤回]', italics: true, color: '999999', size: 18 })],
          spacing: { after: 100 },
        }));
        continue;
      }
      ...
      if (ticket.creatorId === msg.senderId) {
        ...
        roleTag = '协助专家';
        roleColor = '7c3aed'; // 紫
      } else if (msg.sender?.role?.name === 'external' || (typeof msg.senderId === 'string' && String(msg.senderId).startsWith('ext-'))) {
        roleTag = '外部';
        roleColor = 'd97706'; // 橙
      } else {
        roleTag = '内部';
      ...
      docChildren.push(new Paragraph({
        children: [
          new TextRun({ text: `【${senderName}`, bold: true, size: 20, color: roleColor, font: 'Microsoft YaHei' }),
          new TextRun({ text: ` · ${roleTag}`, bold: true, size: 18, color: '888888', font: 'Microsoft YaHei' }),
          new TextRun({ text: `】 ${timeStr}`, size: 18, color: '888888', font: 'Microsoft YaHei' }),
        ],
    ...
    docChildren.push(new Paragraph({
      children: [new TextRun({ text: `本报告由 CallCenter 系统自动生成 · ${new Date().toLocaleString('zh-CN')}`, color: '999999', size: 16, italics: true, font: 'Microsoft YaHei' })],
      alignment: AlignmentType.CENTER,
      spacing: { before: 40 },
    }));
